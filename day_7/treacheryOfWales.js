...
const crabPositionData = await dataParser('./data.txt', ',', data => data.map(position => parseInt(position)));
...
const findBestPositionAndFuelCost = (range, crabPositionData) => {
    let bestPosition;
    let lowestCost;
    ...
    for(let i = range[0]; i <= range[1]; i++) {
        ...
        if(!bestPosition || cost < lowestCost) {
            lowestCost = cost;
            bestPosition = i;
        }
    }
...
const findBestPositionAndFuelCost2 = (range, crabPositionData) => {
    let bestPosition;
    let lowestCost;
